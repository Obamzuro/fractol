...
__kernel void drawFractal(__global const double *input, __global unsigned char *output)
{
	...
	if (input[5] == 5)
	{
		maxdif[0] = (input[1] - input[0]) / WINWIDTH;
		maxdif[1] = (input[3] - input[2]) / WINHEIGHT;
		depth = input[4] / 10;
		i = -1;
		edge = WINWIDTH / 3;
		isblack = 0;
		...
		{
			if (edge < maxdif[0] || edge < maxdif[1])
				break ;
			xfactor = x / edge;
			yfactor = y / edge;
	...
	//MANDELBROT AND ELSE
	{
		...
		while (++depth < input[4])
		{
			...
			{
				nextim = fabs(real * im) * 2;
				nextreal = fabs(realsquared - imsquared);
			}
			else
			{
				nextreal = realsquared - imsquared;
